...
oauth2_scheme = OAuth2AuthorizationCodeBearer(
    ...
    scopes={"openid": "OpenID Connect", "profile": "Profile information", "email": "Email information"}
)
...
async def get_jwks():
    ...
    if jwks_cache["last_updated"] and jwks_cache["keys"] and jwks_cache["last_updated"] > datetime.now() - timedelta(hours=24):
        return jwks_cache["keys"]
    
    # Fetch JWKs from DEX
    async with httpx.AsyncClient() as client:
        response = await client.get(f"{DEX_ISSUER_URL}/.well-known/openid-configuration")
