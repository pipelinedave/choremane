...
def test_status_check(monkeypatch):
    monkeypatch.setattr(
        "app.api.routes.get_db_connection", dummy_get_db_connection_for_status
    )
...
def test_get_logs(monkeypatch):
    monkeypatch.setattr(
        "app.api.routes.get_db_connection", dummy_get_db_connection_for_logs
    )
...
def test_get_chores(monkeypatch):
    monkeypatch.setattr(
        "app.api.routes.get_db_connection", dummy_get_db_connection_for_chores
    )
        ...
        def execute(self, query, params=None):
            ...
            if "SELECT interval_days" in query:
                self._row = (
                    ...
                    today,
                    today,
                )  # interval_days, due_date, last_done=today
    ...
    class DummyCursor:
        ...
        def execute(self, query, params=None):
            if "SELECT action_type" in query:
                ...
                self._row = (
                    "marked_done",
                    json.dumps({"chore_id": 42, "previous_due_date": "2025-04-27"}),
                )
